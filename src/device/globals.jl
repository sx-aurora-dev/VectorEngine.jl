...
# space.
@generated function get_global_pointer(::Val{global_name}, ::Type{T})::VectorEngine.LLVMPtr{T,AS.Global} where {global_name, T}
    Context() do ctx
        ...
        # Call the function.
        quote
            VectorEngine.LLVMPtr{T,AS.Global}(convert(Csize_t, $(call_function(llvm_f, Ptr{T}))))
        end
    end
end
